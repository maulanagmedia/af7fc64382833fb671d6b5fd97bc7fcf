...
public class NavNearby extends Fragment implements LocationListener {
    ...
    private ArrayList<HashMap<String, String>> listSuarMer;
                ...
                try {
                    ...
                    if(iv.parseNullInteger(status) == 200){
                        ...
                        for(int i = 0; i < jsonArray.length();i++){
                            ...
                            /*merchantList.add(new CustomItem("LIST", item.getString("id_m"),item.getString("nama"),item.getString("deskripsi"), item.getString("foto"),item.getString("alamat"), round(item.getString("jarak"),2)));

                            if(i == 0 || i%16 == 0){
                                merchantList.add(new CustomItem("ADV", offerImage, linkOffer));
                            }else if(i%16 == 8){
                                if(offerIndex >= listSuarMer.size()){
                                    offerIndex = 0;
                                }
                                HashMap<String, String> map =  listSuarMer.get(offerIndex);
                                merchantList.add(new CustomItem("ADV", map.get("icon"), map.get("link")));
                                offerIndex++;
                            }*/
                        }
                    }
